    ...
    def get_post_details(self, query, post_urls, directory, post_dir, number):
        ...
        for post in post_urls:
            ...
            if (driver.find_elements_by_css_selector(".coreSpriteRightChevron")):
                while (driver.find_elements_by_css_selector(".SWk3c") or driver.find_elements_by_css_selector(
            ...
            self.save_and_download(num, query, post, caption, likes, date, location, image, comments, tagged_people,
                                   post_dir, post_dict)
        return post_dict

    def save_and_download(self, num, query, post, caption, likes, date, location, image, comments, tagged_people,
                          post_dir, post_dict):
        driver = self.driver
        post_dict[post] = {
            ...
            'year': date.split(' ')[2],
            'time': driver.find_element(By.CSS_SELECTOR, "._1o9PC").get_attribute('datetime').split('T')[1].split('.')[
                0],
            'location': location,
            'images': len(image),
            'creator': query,
...
def main():
    ...
    parser.add_argument('-t', '--crawl_type', type=str, default='posts',
                        help="Options: 'details' | 'posts' | 'followers' | 'following' | 'all'")
    parser.add_argument('-n', '--number', type=int, default=0,
                        help='Number of posts to download: integer, -1 to download all')
